...
public:
    ...
    std::unique_ptr<DCE> build_unique(const config *cfg, Args &&... args)
    {
        ...
        case ModemType::AIR780E:
            return build_unique<AIR780E>(cfg, std::forward<Args>(args)...);
        case ModemType::AIR724U:
            return build_unique<AIR724U>(cfg, std::forward<Args>(args)...);
        case ModemType::GenericModule:
    ...
    DCE *build(const config *cfg, Args &&... args)
    {
        ...
        case ModemType::AIR780E:
            return build<AIR780E>(cfg, std::forward<Args>(args)...);
        case ModemType::AIR724U:
            return build<AIR724U>(cfg, std::forward<Args>(args)...);
        case ModemType::GenericModule:
