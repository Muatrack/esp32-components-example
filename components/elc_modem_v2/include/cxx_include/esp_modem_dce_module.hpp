...
public:
    ...
    void set_module_name(std::string n) {
        module_name = n;
    }

    std::string get_module_name(void) {
        return module_name;
    }
    ...
    command_result wait_ready(uint32_t timeout) override 
    {
        return wait_lte_ready(timeout);
    }

    void poweron() override
    {
        gpio_config_t gt = {
            // .pin_bit_mask = ((1U << powon_pin) | (1U << powoff_pin)),
            .pin_bit_mask = ((1U << powon_pin)),
        ...
        // gpio_set_level((gpio_num_t)powon_pin, 1);  // for elc_board
        // usleep(100000);
        ESP_LOGI("MODEM", "power on");
        gpio_set_level((gpio_num_t)powon_pin, 0);  // for elc_board
        usleep(20000000);
        gpio_set_level((gpio_num_t)powon_pin, 1);  // for elc_board
    }
   ...
   /**
     * 读取模块的网络状态
     * @param reportSt  模块主动通过串口上报网络状态的开关值， 0禁止/关闭自动上报, 1开启上报网络状态。
     * @param netSt     当前模块的网络注册状态。0-未注册网络|未搜到运营商， 1-已注册|本地网络，2-未注册|正在搜索，3-注册被拒绝。 4，5  
     * @return 执行成功OK, 超时 TIMEOUT
    */
    virtual command_result lte_network_state_get(int &reportSt, int &netSt) {
        ESP_LOGE("MODEM_DCE","func need be override.");
     ...
     * 读取基站定位
    */
    virtual command_result lte_network_location_get(std::string &longi, std::string &lati) {
        get_lte_network_location(longi, lati);
        // ESP_LOGI("MODEM_DCE", "longi len:%d, val:%s, lati len:%d, val:%s", longi.size(), longi.c_str(), lati.size(), lati.c_str());
        return command_result::OK;
    }
     ...
     * 重启/复位LTE模块
    */
    virtual void lte_module_reset(void) {
        ESP_LOGE("MODEM_DCE","func need be override.");
    }
...
/**
 * @brief Specific definition of the AIR780E module
 */
class AIR780E: public GenericModule {
    using GenericModule::GenericModule;
public:
    ~AIR780E() {
    };
public:
    command_result set_data_mode() override;
    bool setup_data_mode() override;
    command_result power_reup() override;
    // command_result get_signal_quality(int&, int&);
    // void poweron() override;
    command_result lte_network_state_get(int &reportSt, int &netSt) override;
    void lte_module_reset(void) override;
    
private:
    std::unique_ptr<LTEPowGpio> pLTEGpio;
};

/**
 * @brief Specific definition of the AIR724U module
 */
class AIR724U: public GenericModule {
    using GenericModule::GenericModule;
};
/**
 * @}
