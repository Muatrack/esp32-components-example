...
std::unique_ptr<DCE> create_AIR780E_dce(const dce_config *config, std::shared_ptr<DTE> dte, esp_netif_t *netif)
{
    return create_modem_dce(dce_factory::ModemType::AIR780E, config, std::move(dte), netif);
}

std::unique_ptr<DCE> create_AIR724U_dce(const dce_config *config, std::shared_ptr<DTE> dte, esp_netif_t *netif)
{
    return create_modem_dce(dce_factory::ModemType::AIR724U, config, std::move(dte), netif);
}
